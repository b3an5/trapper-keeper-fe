  ...
  render() {
    ...
    const completeList = completeListItems.map((li, i) => {
      ...
      return (
        ...
        >
          <img src={checkbox} alt="checked checkbox" className="checked-icon" />
          <p className="completed-li-text list-text">{li.text}</p>
        </li>
      )
    })
    const incompleteList = incompleteListItems.map(li => {
      return (
        <li key={li.id} className="incomplete-li" id={`item-${li.id}`}>
          <button
            className="note-complete-btn round-btn"
            id={`${li.id}-btn`}
            onClick={() => this.props.toggleCompletedLi(li)}
          >
            <img src={checkbox} alt="" className="checkbox-hover" />
          </button>
          <label className="list-text" htmlFor={`${li.id}-btn`}>
            {li.text}
          </label>
          <button className="delete-li-btn" onClick={this.deleteLi}>
      ...
      <article className="note-card">
        ...
        {incompleteList.length === 0 && (
          <p className="empty-list-message">
            You completed everything on your list! Cheers!
          </p>
        )}
        <ul className="incomplete-ul">
          ...
          <Link to={`/notes/${id}`}>
            <div className="add-wrapper">
              <img src={addNew} className="add-new-icon round-btn" alt="" />
              <p className="add-new-text">Add New</p>
            </div>
          </Link>
        </ul>
        {completeList[0] && <h5 className="complete-heading">complete</h5>}

        <ul className="complete-ul">{completeList}</ul>
